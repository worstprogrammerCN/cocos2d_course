...
bool HelloWorld::init()
{
	...
	schedule(schedule_selector(HelloWorld::createAndMoveMonster), 2.0f);
...
void HelloWorld::labelCallback(Ref* pSender, std::string direction)
{
	...
	if (!playerIsAnimating
		...
		if (direction == "W") {
			x = 0;
			y = player->getContentSize().height * 3 / 7;
		}
		else if (direction == "A"){
			if (lastCid != 'A')
				player->setFlippedX(true);
			lastCid = 'A';
			x = -player->getContentSize().width * 3 / 7;
			y = 0;
		}
		else if (direction == "S") {
			x = 0;
...
void HelloWorld::attackCallback(Ref * pSender)
{
	...
	if (!playerIsAnimating) {
		...
		if (collision != NULL && percentage < 100) {
			auto to = ProgressFromTo::create(0.85f, percentage, percentage + 20);
			pT->runAction(to);
		}
	}
...
void HelloWorld::hitByMonster(float dt)
{
	...
	Rect attackRect = Rect(playerRect.getMinX() - 40, playerRect.getMinY() - 20,
		playerRect.getMaxX() - playerRect.getMinX() + 80, playerRect.getMaxY() - playerRect.getMinY() + 40);
	Sprite *beingAttackCol = fac->collider(attackRect);
	if (beingAttackCol != NULL) {
		if (attacking) {
			fac->removeMonster(beingAttackCol);
